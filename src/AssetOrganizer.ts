...
export class AssetOrganizer {
  ...
  constructor(cache: AssetCache, usageTracker: UsageTracker) {
    this.cache = cache;
   ...
   */
  async moveAsset(asset: CachedAsset, newPath: string): Promise<boolean> {
   ...
   * @returns Promise<boolean> - Success status
   */
  async renameAsset(asset: CachedAsset, newName: string): Promise<boolean> {
   ...
   */
  async deleteAssets(assets: CachedAsset[]): Promise<{
    success: number;
    failed: number;
    totalSize: number;
  }> {
   ...
   */
  async dryRunDelete(assets: CachedAsset[]): Promise<{
    assets: CachedAsset[];
    totalSize: number;
    totalCount: number;
  }> {
   ...
   */
  private async moveFile(oldPath: string, newPath: string): Promise<boolean> {
