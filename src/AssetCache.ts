...
import { AssetMetadata, CachedAsset, SearchCriteria, UsageInfo } from './types';

export class AssetCache {
  private db: IDBDatabase | null = null;
  private readonly dbName = 'AssetAtlasDB';
      ...
      try {
        ...
        request.onupgradeneeded = (event) => {
          try {
            ...
            reject(new Error(`Failed to create database schema: ${(error as Error).message}`));
          }
        };
        ...
        reject(new Error(`Database initialization error: ${(error as Error).message}`));
      }
    });
  }
   ...
   */
  async upsertAsset(metadata: AssetMetadata): Promise<void> {
    if (!this.db) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        getRequest.onsuccess = () => {
          try {
            const existingAsset = getRequest.result as CachedAsset | undefined;

            const cachedAsset: CachedAsset = existingAsset
              ? {
                  ...existingAsset,
                  ...metadata,
                  indexed: Date.now()
            ...
            reject(new Error(`Failed to prepare asset data: ${(error as Error).message}`));
          }
        };
        ...
        reject(new Error(`Unexpected error: ${(error as Error).message}`));
      }
    });
  }
   ...
   */
  async getAsset(path: string): Promise<CachedAsset | null> {
    if (!this.db) {
   ...
   */
  async removeAsset(path: string): Promise<boolean> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        const asset = getRequest.result as CachedAsset | undefined;
   ...
   */
  async searchAssets(criteria: SearchCriteria): Promise<CachedAsset[]> {
    if (!this.db) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onsuccess = () => {
          try {
            let results = request.result as CachedAsset[];
            ...
            resolve(request.result as CachedAsset[]);
          }
        };
   ...
   */
  async updateUsage(path: string, usage: UsageInfo): Promise<void> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
   ...
   */
  private applyFilters(assets: CachedAsset[], criteria: SearchCriteria): CachedAsset[] {
    let filtered = assets;
