...
export class CustomImportDialog extends Application {
  private destination: 'world' | 'global';
  ...
  constructor(options: ImportDialogOptions) {
    ...
    this.worldName = ((game as any).world?.id || (game as any).world?.name || options.worldName || 'default');
  ...
  private _onBrowseClick(event: JQuery.TriggeredEvent): void {
    event.preventDefault();
    const fileInput = this.element.find('#file-input').get(0) as HTMLInputElement;
    fileInput?.click();
  }

  private _onFileSelect(event: JQuery.TriggeredEvent): void {
  ...
  private _onClearFiles(event: JQuery.TriggeredEvent): void {
    event.preventDefault();
    this.selectedFiles = [];
    const fileInput = this.element.find('#file-input').get(0) as HTMLInputElement;
    if (fileInput) {
  ...
  private _onDirectoryChange(event: JQuery.TriggeredEvent): void {
    this.targetDirectory = $(event.currentTarget).val() as string;
  }
  ...
  private async _onImport(event: JQuery.TriggeredEvent): Promise<void> {
    event.preventDefault();
    ...
    try {
      ...
      if (successCount > 0) {
        ui.notifications?.info(
          `Successfully imported ${successCount} file(s)${failCount > 0 ? `. Failed: ${failCount}` : ''}. Refreshing...`
        );
      } else {
  ...
  private async uploadFile(file: File, targetDirectory: string): Promise<string> {
    // In a real Foundry implementation, use FilePicker.upload
    
    if (typeof (FilePicker as any) !== 'undefined' && (FilePicker as any).upload) {
      try {
        // First, try to create the directory if it doesn't exist
        try {
          await (FilePicker as any).createDirectory('data', targetDirectory, {});
        ...
        const result = await (FilePicker as any).upload(
          'data',
          targetDirectory,
          file,
          {},
  ...
  private getAssetType(fileName: string): 'image' | 'audio' | 'video' {
  ...
  static async show(options: ImportDialogOptions): Promise<void> {
